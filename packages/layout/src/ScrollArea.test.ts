...
describe('ScrollArea', () => {
  ...
  it('root element has no tabindex', () => {
    const wrapper = mount(ScrollArea)
    expect(wrapper.attributes('tabindex')).toBeUndefined()
  })

  it('viewport has a unique ID for ARIA controls linkage', () => {
    const wrapper = mount(ScrollArea)
    const viewport = wrapper.find('[data-rig-scroll-viewport]')
    expect(viewport.attributes('id')).toContain('rig-scroll-viewport')
  })

  it('does not emit custom events on scroll', async () => {
  ...
  it('preserves viewport when scrollbar props change', async () => {
    const wrapper = mount(ScrollArea, { props: { vertical: true } })
    expect(wrapper.find('[data-rig-scroll-viewport]').exists()).toBe(true)
    await wrapper.setProps({ vertical: false, horizontal: true })
    expect(wrapper.find('[data-rig-scroll-viewport]').exists()).toBe(true)
  })
  ...
  describe('scroll metrics and scrollbar rendering', () => {
    ...
    it('computes thumb height percentage from scroll ratio', async () => {
      ...
      const thumb = wrapper.find(
        '[data-rig-scroll-bar][data-orientation="vertical"] [data-rig-scroll-thumb]',
      )
    ...
    it('thumb width percentage for horizontal scroll', async () => {
      ...
      const thumb = wrapper.find(
        '[data-rig-scroll-bar][data-orientation="horizontal"] [data-rig-scroll-thumb]',
      )
      expect(thumb.exists()).toBe(true)
      // 500/2000 = 25%
