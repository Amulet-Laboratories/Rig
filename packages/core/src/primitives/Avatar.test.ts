...
describe('Avatar', () => {
  ...
  it('renders status indicator when status prop is set', () => {
    const wrapper = mount(Avatar, { props: { name: 'Test', status: 'online' } })
    expect(wrapper.find('[data-rig-avatar-status]').exists()).toBe(true)
  })

  it('updates aria-label when name prop changes', async () => {
    const wrapper = mount(Avatar, { props: { name: 'Test' } })
