              ...
              >
                <template #tab="{ tab }">
                  <span class="ide-tab-inner" @contextmenu="(e: MouseEvent) => onTabContextMenu(e, tab.id as string)">
                    ...
                    <span v-if="tab.closable !== false" data-rig-editor-tab-close aria-label="Close" tabindex="-1" @click.stop="closeTab(tab.id as string)">
                      &times;
                    </span>
                  </span>
                </template>
...
.ide-titlebar {
  ...
  height: 30px;
  min-height: 30px;
  background: var(--color-card, #323233);
...
.ide-sidebar-title {
  ...
  color: var(--color-muted-foreground, #bbb);
}
...
.ide-panel-area {
  ...
  height: 100%;
  background: var(--color-background, #1e1e1e);
  border-top: 1px solid color-mix(in srgb, var(--color-foreground, #fff) 6%, transparent);
}
...
.ide-terminal-toggle {
  ...
  border: none;
  color: color-mix(in srgb, var(--color-foreground, #fff) 70%, transparent);
  cursor: pointer;
  padding: 0;
  font-size: 14px;
}
...
.ide-terminal-toggle:hover {
  ...
  background: color-mix(in srgb, var(--color-primary, #007acc) 100%, #fff 15%);
}
...
.ide-search-input {
  width: 100%;
  padding: 4px 8px;
  background: var(--color-card, #3c3c3c);
  border: 1px solid color-mix(in srgb, var(--color-foreground, #fff) 10%, transparent);
  border-radius: 4px;
  color: var(--color-foreground, #ccc);
  font-size: 13px;
  outline: none;
